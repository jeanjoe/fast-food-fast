...
if (signin_form) {
    ...
    signin_form.addEventListener('submit', function(event){
        ...
        fetch(BASE_API_URL + "admins/login", {
            //Fetch data from api
            method: "POST",
        ...
        .then(function(responseJson) {
            ...
            } else if (responseJson.admin_token) {
                //Add token to cookie
                createCookie('access_token',responseJson.admin_token, 30)
                login_info.classList.remove('alert-danger')
...
if (sign_up_form) {
    ...
    sign_up_form.addEventListener('submit', function(event){
        ...
        if (register_password !== register_confirm_password) {
            ...
            //Fetch data from url
            fetch(BASE_API_URL + "admins/register", {
                method: "POST",
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({
            ...
            .then(function(responseJson) {
                ...
                if (responseJson.errors) {
                ...
                } else if (responseJson.message == "Admin registered successfully") {
                    ...
                    redirectToUrl('/admin/login', 5000)
                } else {
                    //Return generaic error message
